...
void D3DContext::Init(HWND hWnd, WindowProperties windowPropertise) {
	...
	auto hr_factory = CreateDXGIFactory2(DXGI_CREATE_FACTORY_DEBUG, __uuidof(IDXGIFactory7), (void**)&pFactory);

	std::vector<DXGI_ADAPTER_DESC3> dapters;
	std::vector<IDXGIAdapter4*> adapters;
	IDXGIAdapter* pAdapter = nullptr;

	for (UINT i = 0; pFactory->EnumAdapters(i, &pAdapter) != DXGI_ERROR_NOT_FOUND; ++i) {
		IDXGIAdapter4* pAdapter4 = nullptr;
		pAdapter->QueryInterface(__uuidof(IDXGIAdapter4), (void**)&pAdapter4);

		adapters.push_back(pAdapter4);
